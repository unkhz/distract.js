...
(function(root, factory) {
    ...
    function extend() {
        ...
        for (i = 0, len = rest.length; i < len; i++) {
            ...
            if (src) {
                for (j in src) {
                    if (Object.prototype.toString.call(dest[j]) === "[object Object]" && Object.prototype.toString.call(src[j]) === "[object Object]") {
                        extend(dest[j], src[j]);
                    } else {
                        dest[j] = src[j];
                    }
    ...
    arraySlice = Array.prototype.slice, callbackParticleOpts = [ "initState", "iterateState", "parseIterationRule" ], defaultLayerOpts = {
    ...
    }, testEl = document.createElement("div"), propertyCache = {}, requestAnimationFrame = root.requestAnimationFrame || root.webkitRequestAnimationFrame || root.mozRequestAnimationFrame || function(cb) {
        setTimeout(cb, 1);
    };
    ...
    function buildDimensionString(valueObject, aMethod) {
        var val = valueObject[0], suffix = valueObject[1];
    ...
    Layer.prototype.animate = function() {
        this.paused = false;
        var _this = this, createParticle = function() {
            if (_this.paused) {
                return;
            }
