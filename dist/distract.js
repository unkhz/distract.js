 ...
 */
    function extend() {
        var dest = arguments[0], rest = Array.prototype.slice.call(arguments, 1), i, j, len;
        for (i = 0, len = rest.length; i < len; i++) {
            var src = rest[i];
            if (src) {
                for (j in src) {
                    if (objectToString.call(dest[j]) === "[object Object]" && objectToString.call(src[j]) === "[object Object]") {
                        extend(dest[j], src[j]);
                    } else {
                        dest[j] = src[j];
                    }
 ...
 * @private
 */
    var requestAnimationFrame = root.requestAnimationFrame || root.webkitRequestAnimationFrame || root.mozRequestAnimationFrame || root.oRequestAnimationFrame || root.msRequestAnimationFrame || function(cb) {
        setTimeout(cb, 1);
    };
    /**
 ...
 * @private
 */
    var cancelAnimationFrame = root.cancelAnimationFrame || root.webkitCancelRequestAnimationFrame || root.mozCancelRequestAnimationFrame || root.oCancelRequestAnimationFrame || root.msCancelRequestAnimationFrame || clearTimeout;
    /**
 ...
 */
    function pickParticleOpts(id, opts) {
        ...
        for (key in opts) {
            val = opts[key];
            val = val.join ? val[Math.ceil(Math.random() * val.length) - 1] : val;
 ...
 */
     ...
     */
        parseIterationRule: function(rule, state, opts, particle) {
            var property = getValue(rule, "property", arguments), propertyFunction = getValue(rule, "propertyFunction", arguments), inc = getValue(rule, "inc", arguments), speed = getValue(rule, "speed", arguments, 1), max = getValue(rule, "max", arguments, Infinity), min = getValue(rule, "min", arguments, -Infinity), oldPropertyObject = state.style[property] = state.style[property] || [], oldValue = propertyFunction ? oldPropertyObject[propertyFunction][0] : oldPropertyObject[0], newValue = Math.min(Math.max(oldValue + inc * speed, min), max);
            if (propertyFunction) {
     ...
     */
        animate: function() {
            ...
            var createParticle = function() {
                ...
                var id, el, p, pId, opts = _this.opts, len = _this.particles.length;
                ...
                if (len > 0) {
                    pId = ++_this.particleCheckId % _this.particles.length, p = _this.particles[pId];
                    if (p && p.isDestroyed) {
